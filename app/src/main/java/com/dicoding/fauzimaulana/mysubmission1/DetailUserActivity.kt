...
import com.dicoding.fauzimaulana.mysubmission1.databinding.ActivityDetailUserBinding
        ...
        binding.fabFavorite.setOnClickListener {
            if (statusFavorite) {
                ...
                val values = contentValuesOf(
                    UserColumns.NAME to user?.name,
                    UserColumns.FOLLOWERS to user?.followers,
                    UserColumns.FOLLOWING to user?.following,
                    UserColumns.AVATAR to user?.avatar,
                    UserColumns.USERNAME to user?.username,
                    UserColumns.COMPANY to user?.company,
                    UserColumns.LOCATION to user?.location,
                    UserColumns.REPOSITORY to user?.repository
                )
